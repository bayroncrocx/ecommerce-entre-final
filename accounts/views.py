...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('/accounts/login/?command=verification&email='+email, csrf_protect)
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.success(request, 'has iniciado sesion con exitosamente')
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user,token):
        user.is_active = True
        user.save()
        messages.success(request, 'felicidades su cuenta a sido activada!')
        return redirect('login')
    else:
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.error(request, 'El link ha espirado')
        return redirect('login')
